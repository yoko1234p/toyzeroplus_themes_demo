...
const GridBox: React.FC<{
  ...
  delay?: string;
  onClick?: () => void;
}> = ({ children, className = "", delay = "0s", onClick }) => {
  const { ref, isVisible } = useScrollReveal();
  return (
    <div
      ref={ref}
      onClick={onClick}
      className={`relative bg-white transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"} ${className}`}
...
const SealModeSections: React.FC<SealModeSectionsProps> = ({
  ...
  const products =
    propProducts && propProducts.length > 0 ? propProducts : PRODUCTS;
        ...
        {products.map((p, i) => (
          ...
          >
            ...
            <div className="flex-grow border-2 border-[#EAEAEA] group-hover:border-[#B08D57] transition-colors duration-500 relative bg-[#F9F9F9] p-2">
              {/* Type Tag - 從 custom.hppye_tag metafield 讀取 */}
              {(p as Product).tag && (
              ...
              <div className="h-full border border-[#333]/10 p-4 flex flex-col relative overflow-hidden">
                ...
                <div className="text-center mt-auto">
                  <h3 className="text-2xl font-lhkk font-black text-[#333] mb-1 h-8 flex items-center justify-center">
                    {p.name}
                  </h3>
                  <p className="text-[10px] text-[#B08D57] tracking-[0.2em] uppercase mb-4 font-mono h-8 flex items-center justify-center">
                    {(p as any).nameEn || ""}
                  </p>
                ...
                <div className="mt-4 pt-4 border-t border-dashed border-[#333]/20 flex justify-between items-center">
                  <span className="font-mono text-lg font-bold text-[#333]">
                    {(p as any).formattedPrice ||
                      (typeof p.price === "number" ? `HK$${p.price}` : p.price)}
                  </span>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
      ...
      <section className="grid grid-cols-1 md:grid-cols-12 gap-0 border-t border-b border-[#B08D57]/30">
        ...
        <div className="md:col-span-9 p-8 md:p-12 flex flex-col justify-center bg-white/90 backdrop-blur-sm">
          <div className="flex items-center gap-4 mb-6">
            <span className="text-[#B08D57] font-mono text-xs tracking-[0.3em] uppercase border border-[#B08D57] px-2 py-1">
